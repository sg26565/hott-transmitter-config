	...
	<tbody>
		<#list model.dualMixer as mix>
			<tr class="<@d/> <@u mix.channel??/>">
				...
				<td align="center">&uarr;<#if mix.channel??>${mix.channel[0].number?number+1}<#if mix.channel[0].function??> (${mix.channel[0].function})</#if><#else>??</#if>&uarr;</td>
				<td align="center">&uarr;<#if mix.channel??>${mix.channel[1].number?number+1}<#if mix.channel[1].function??> (${mix.channel[1].function})</#if><#else>??</#if>&darr;</td>
