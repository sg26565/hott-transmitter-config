 ...
 * @see <a href="https://github.com/mik3y/usb-serial-for-android">USB Serial for
 *      Android project page</a>
...
public class FtdiSerialDriver implements UsbSerialDriver {
    ...
    private class FtdiSerialPort extends CommonUsbSerialPort {
        ...
        /**
         * Due to http://b.android.com/28023 , we cannot use UsbRequest async
         * reads since it gives no indication of number of bytes read. Set this
         * to {@code true} on platforms where it is fixed.
         */
         ...
         * @param dest
         *            The destination buffer to write the status bytes into (can
         *            be src)
        ...
        private final int filterStatusBytes(final byte[] src, final byte[] dest, final int totalBytesRead, final int maxPacketSize) {
            ...
            for (int packetIdx = 0; packetIdx < packetsCount; ++packetIdx) {
                final int count = packetIdx == packetsCount - 1 ? totalBytesRead % maxPacketSize - MODEM_STATUS_HEADER_LENGTH : maxPacketSize
                        - MODEM_STATUS_HEADER_LENGTH;
                if (count > 0) {
                    System.arraycopy(src, packetIdx * maxPacketSize + MODEM_STATUS_HEADER_LENGTH, dest, packetIdx
        ...
        public void reset() throws IOException {
            final int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST, SIO_RESET_SIO, 0 /* index */, null, 0,
        ...
        private int setBaudRate(final int baudRate) throws IOException {
            final long[] vals = convertBaudrate(baudRate);
            final long actualBaudrate = vals[0];
            final long index = vals[1];
            final long value = vals[2];
            final int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_SET_BAUD_RATE_REQUEST, (int) value, (int) index, null, 0,
                    USB_WRITE_TIMEOUT_MILLIS);
            if (result != 0) {
        ...
        public void setParameters(final int baudRate, final int dataBits, final int stopBits, final int parity) throws IOException {
            ...
            final int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_SET_DATA_REQUEST, config, 0 /* index */, null, 0,
                    USB_WRITE_TIMEOUT_MILLIS);
            if (result != 0) {
        ...
        private long[] convertBaudrate(final int baudrate) {
                ...
                if (tryDivisor <= 8) {
                    ...
                    if (mType == DeviceType.TYPE_AM) {
                        ...
                        if (tryDivisor > 0x1FFFF) {
                            // Round down to maximum supported divisor value
                            // (for
                            // BM)
                            tryDivisor = 0x1FFFF;
                        }
            ...
            long encodedDivisor = bestDivisor >> 3 | fracCode[bestDivisor & 7] << 14;
                ...
                final long value = encodedDivisor & 0xFFFF;
                long index;
        ...
        public boolean purgeHwBuffers(final boolean purgeReadBuffers, final boolean purgeWriteBuffers) throws IOException {
            if (purgeReadBuffers) {
                final int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST, SIO_RESET_PURGE_RX, 0 /* index */, null, 0,
                        USB_WRITE_TIMEOUT_MILLIS);
                if (result != 0) {
            ...
            if (purgeWriteBuffers) {
                final int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST, SIO_RESET_PURGE_TX, 0 /* index */, null, 0,
                        USB_WRITE_TIMEOUT_MILLIS);
                if (result != 0) {
                    throw new IOException("Flushing RX failed: result=" + result);
                }
