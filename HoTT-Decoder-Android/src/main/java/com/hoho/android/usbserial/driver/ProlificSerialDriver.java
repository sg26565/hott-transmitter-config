...
public class ProlificSerialDriver implements UsbSerialDriver {
    ...
    class ProlificSerialPort extends CommonUsbSerialPort {
        ...
        private final byte[] inControlTransfer(final int requestType, final int request, final int value, final int index, final int length) throws IOException {
            ...
            final int result = mConnection.controlTransfer(requestType, request, value, index, buffer, length, USB_READ_TIMEOUT_MILLIS);
            if (result != length) {
        ...
        private final void outControlTransfer(final int requestType, final int request, final int value, final int index, final byte[] data) throws IOException {
            ...
            final int result = mConnection.controlTransfer(requestType, request, value, index, data, length, USB_WRITE_TIMEOUT_MILLIS);
            if (result != length) {
                throw new IOException(String.format("ControlTransfer with value 0x%x failed: %d", value, result));
            }
        }
        ...
        private final void readStatusThreadFunction() {
            try {
                while (!mStopReadStatusThread) {
                    final byte[] buffer = new byte[STATUS_BUFFER_SIZE];
                    final int readBytesCount = mConnection.bulkTransfer(mInterruptEndpoint, buffer, STATUS_BUFFER_SIZE, 500);
                    if (readBytesCount > 0) {
                        if (readBytesCount == STATUS_BUFFER_SIZE) {
                            mStatus = buffer[STATUS_BYTE_IDX] & 0xff;
                        } else {
                            throw new IOException(String.format("Invalid CTS / DSR / CD / RI status buffer received, expected %d bytes, but received %d",
                                    STATUS_BUFFER_SIZE, readBytesCount));
                        }
                    }
                }
        ...
        private final int getStatus() throws IOException {
            if (mReadStatusThread == null && mReadStatusException == null) {
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread == null) {
                        final byte[] buffer = new byte[STATUS_BUFFER_SIZE];
                        final int readBytes = mConnection.bulkTransfer(mInterruptEndpoint, buffer, STATUS_BUFFER_SIZE, 100);
            ...
            /*
             * throw and clear an exception which occured in the status read
             * thread
             */
            final IOException readStatusException = mReadStatusException;
        ...
        public void open(final UsbDeviceConnection connection) throws IOException {
            ...
            try {
                ...
                if (mDevice.getDeviceClass() == 0x02) {
                    ...
                    try {
                        final Method getRawDescriptorsMethod = mConnection.getClass().getMethod("getRawDescriptors");
                        final byte[] rawDescriptors = (byte[]) getRawDescriptorsMethod.invoke(mConnection);
                        ...
                        Log.w(TAG, "Method UsbDeviceConnection.getRawDescriptors, " + "required for PL2303 subtype detection, not "
                                + "available! Assuming that it is a HX device");
                        mDeviceType = DEVICE_TYPE_HX;
                    } catch (final Exception e) {
                        Log.e(TAG, "An unexpected exception occured while trying " + "to detect PL2303 subtype", e);
                    }
                }
        ...
        public int read(final byte[] dest, final int timeoutMillis) throws IOException {
            synchronized (mReadBufferLock) {
                ...
                final int numBytesRead = mConnection.bulkTransfer(mReadEndpoint, mReadBuffer, readAmt, timeoutMillis);
                if (numBytesRead < 0) {
                    return 0;
                }
        ...
        public void setParameters(final int baudRate, final int dataBits, final int stopBits, final int parity) throws IOException {
            if (mBaudRate == baudRate && mDataBits == dataBits && mStopBits == stopBits && mParity == parity) {
                // Make sure no action is performed if there is nothing to
                // change
                return;
            }

            final byte[] lineRequestData = new byte[7];
