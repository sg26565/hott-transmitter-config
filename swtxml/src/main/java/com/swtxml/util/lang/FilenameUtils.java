...
public class FilenameUtils {
  ...
  private static String doNormalize(final String filename, final boolean keepSeparator) {
    ...
    final char[] array = new char[size + 2]; // +1 for possible extra slash, +2
    // for arraycopy
   ...
   */
  public static boolean isExtension(final String filename, @SuppressWarnings("rawtypes") final Collection extensions) {
    if (filename == null) {
      return false;
    }
    ...
    final Iterator it = extensions.iterator(); it.hasNext();) {
   ...
   * single slash (but UNC names are handled). A single dot path segment will be
   * removed. A double dot will cause that path segment and the one before to be
   ...
   * slash (but UNC names are handled). A single dot path segment will be
   * removed. A double dot will cause that path segment and the one before to be
   * removed. If the double dot has no parent path segment to work with,
   * <code>null</code> is returned.
   * <p>
