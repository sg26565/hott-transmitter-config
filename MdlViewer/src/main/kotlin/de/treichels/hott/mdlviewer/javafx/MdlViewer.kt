...
class MainView : View() {
     ...
     */
    private fun exit() {
        if (runningTask?.isRunning == true) runningTask?.cancel()
        Platform.exit()
    }
    ...
    private fun loadFromFile() {
        FileChooser().apply {
            ...
            extensionFilters.add(FileChooser.ExtensionFilter(messages["mdl_file"], "*.mdl"))
    ...
    private fun loadFromMemory() {
        runningTask = selectFromMemory.openDialog()?.apply { refresh(this) }
    }
    ...
    private fun loadFromSdCard() {
        runningTask = selectFromSDCard.openDialog()?.apply { refresh(this) }
    }
    ...
    /**
     * Refresh [WebView] with the task of the specified [Task]
     */
    private fun refresh(task: Task<Model>) {
        webView.runAsyncWithOverlay {
            task.get()
        }.apply {
            disableUI(runningProperty())
    ...
    private fun saveFile() {
        FileChooser().apply {
            ...
            extensionFilters.add(FileChooser.ExtensionFilter(messages["html_file"], "*.html"))
            extensionFilters.add(FileChooser.ExtensionFilter(messages["pdf_file"], "*.pdf"))
            extensionFilters.add(FileChooser.ExtensionFilter(messages["xml_file"], "*.xml"))
            extensionFilters.add(FileChooser.ExtensionFilter(messages["mdl_file"], "*.mdl"))
