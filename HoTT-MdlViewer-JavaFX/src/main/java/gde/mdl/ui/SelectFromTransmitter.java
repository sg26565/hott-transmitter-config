...
public abstract class SelectFromTransmitter extends Dialog<Task<Model>> {
    ...
    /**
     * Compute the result for the currently selected item. May retrun <code>null</code> if no suitable item was selected or the cancel button was pressed.
     *
     * @param buttonType
     *            The button type of the button that was pressed to close the dialog.
    ...
    protected void handleDoubleClick(final MouseEvent evt) {
        if (evt.getButton() == MouseButton.PRIMARY && evt.getClickCount() == 2 && hasResult()) // do as if the user clicked the load button
            ((Button) getDialogPane().lookupButton(loadButtonType)).fire();
    }

    /**
     * Check whether a suitable item was selected. Subclasses need to define what actually is "suitable".
     *
     * @return
     */
    ...
    private void loadPrefs() {
        ...
        if (portName != null && portName.length() > 0 && portNames.contains(portName)) comboBox.setValue(portName);
    }
    ...
    private void storePrefs() {
        ...
        if (portName != null && portName.length() > 0 && portNames.contains(portName)) Controller.PREFS.put("portName", portName);
    }
}