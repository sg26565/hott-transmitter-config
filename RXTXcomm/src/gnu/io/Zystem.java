...
public class Zystem {
    ...
    static {
        /*
         * The rxtxZystem library uses Python code and is not included with RXTX. A seperate library will be released to avoid potential license conflicts.
    ...
    public Zystem() throws UnSupportedLoggerException {
        final String s = System.getProperty("gnu.io.log.mode");
        if (s != null) {
    ...
    public Zystem(final int m) throws UnSupportedLoggerException {
        mode = m;
        startLogger("asdf");
    }
    ...
    public void filewrite(final String s) {
        try {
            final RandomAccessFile w = new RandomAccessFile(target, "rw");
            w.seek(w.length());
            w.writeBytes(s);
            w.close();
    ...
    public boolean reportln() {
        if (mode == NET_MODE) {
            ...
            filewrite("\n");
        else if (mode == J2EE_MSG_MODE) return false;
        return false;
    }
    ...
    public boolean reportln(final String s) {
        if (mode == NET_MODE) {
            ...
            return false;
        else if (mode == J2SE_LOG_MODE) return true;
        return false;
    }

    public void startLogger() throws UnSupportedLoggerException {
        if (mode == SILENT_MODE || mode == PRINT_MODE) // nativeNetInit( );
            return;
        throw new UnSupportedLoggerException("Target Not Allowed");
    }

    public void startLogger(final String t) throws UnSupportedLoggerException {
        target = t;
        /*
