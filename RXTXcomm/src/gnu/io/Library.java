...
public class Library {
	...
	public static void loadLibrary(final String name, final boolean mapName) {
		...
		if (mapName && load(libName)) {
			return;
		}

		/*
		 * Try loading library from the user.dir directory which is the
		 * directory where application gets started
		 */
		...
		if (load(path + SEPARATOR + mappedName)) {
			return;
		}
		...
		if (path != null) {
			// System.out.println("...extracting library to java.io.tmpdir : " +
			// osPath + mappedName);
			if (extract(path + SEPARATOR + mappedName, osPath + mappedName)) {
				return;
			}
		}
