...
final public class RXTXPort extends SerialPort {
	...
	*/
	...
	class SerialInputStream extends InputStream {
		 ...
		 *      blocks until timeout occurs, returns -1 on timeout >0 >0 blocks
		 *      until timeout, returns - 1 on timeout, magnitude of threshold
		 *      doesn't play a role. 0 >0 Blocks until 1 byte, magnitude of
		 *      threshold doesn't play a role
		 */
		@Override
		 ...
		 *             timeout threshold Behavior
		 *             ------------------------------------------------------------------------
		 *             0 0 blocks until 1 byte is available >0 0 blocks until
		 *             timeout occurs, returns 0 on timeout >0 >0 blocks until
		 *             timeout or reads threshold bytes, returns 0 on timeout 0
		 *             >0 blocks until reads threshold bytes
		 */
		@Override
		 ...
		 *             timeout occurs, returns 0 on timeout >0 >0 blocks until
		 *             timeout or reads threshold bytes, returns 0 on timeout 0
		 *             >0 blocks until either threshold # of bytes or len bytes,
		 *             whichever was lower.
		 */
		@Override
		public synchronized int read(final byte b[], final int off, final int len) throws IOException {
			...
			try {
				...
				if (debug_read_results) {
					z.reportln("RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") returned " + result
							+ " bytes" /* + new String(b) */);
				}
				return result;
			} finally {
	...
	*/
	...
	public void close() {
		synchronized (this) {
			if (debug) {
				z.reportln("RXTXPort:close( " + name + " )");
			}
		...
		setDSR(false);
		if (debug) {
			z.reportln("RXTXPort:close( " + name + " ) setting monThreadisInterrupted");
		}
		if (!monThreadisInterrupted) {
			removeEventListener();
		}
		if (debug) {
			z.reportln("RXTXPort:close( " + name + " ) calling nativeClose");
		}
		nativeClose(name);
		if (debug) {
			z.reportln("RXTXPort:close( " + name + " ) calling super.close");
		}
		super.close();
		fd = 0;
		closeLock = false;
		if (debug) {
			z.reportln("RXTXPort:close( " + name + " ) leaving");
		}
	}
