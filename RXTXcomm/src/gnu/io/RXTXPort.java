...
final public class RXTXPort extends SerialPort {
    ...
    */
    class MonitorThread extends Thread {
        /**
         * Note: these have to be separate boolean flags because the SerialPortEvent constants are NOT bit-flags, they are just defined as integers from 1 to 10
         * -DPL
         */
    ...
    class SerialInputStream extends InputStream {
         ...
         *
         *      timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte is available timeout
         *      > 0, threshold = 0, blocks until timeout occurs, returns -1 on timeout >0 >0 blocks until timeout, returns - 1 on timeout, magnitude of
         *      threshold doesn't play a role. 0 >0 Blocks until 1 byte, magnitude of threshold doesn't play a role
         */
        @Override
        public synchronized int read() throws IOException {
            if (debug_read) z.reportln("RXTXPort:SerialInputStream:read() called");
            if (fd == 0) throw new IOException();
            if (monThreadisInterrupted) z.reportln("+++++++++ read() monThreadisInterrupted");
            synchronized (IOLockedMutex) {
                IOLocked++;
            }
            try {
         ...
         * @throws IOException
         *
         *             timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte is available
         *             >0 0 blocks until timeout occurs, returns 0 on timeout >0 >0 blocks until timeout or reads threshold bytes, returns 0 on timeout 0 >0
         *             blocks until reads threshold bytes
         */
        @Override
         ...
         *             timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte is available
         *             >0 0 blocks until timeout occurs, returns 0 on timeout >0 >0 blocks until timeout or reads threshold bytes, returns 0 on timeout 0 >0
         *             blocks until either threshold # of bytes or len bytes, whichever was lower.
         */
        @Override
        public synchronized int read(final byte b[], final int off, final int len) throws IOException {
            if (debug_read) z.reportln("RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") called" /* + new String(b) */ );
            int result;
            /*
             * Some sanity checks
             */
            ...
            if (threshold == 0) {
                /*
                 * If threshold is disabled, read should return as soon as data are available (up to the amount of available bytes in order to avoid blocking)
                 * Read may return earlier depending of the receive time out.
                 */
                final int a = nativeavailable();
                if (a == 0)
                    Minimum = 1;
                else
         ...
         *
         *             if the read encounters the two bytes, it will return and the array will contain the terminator. Otherwise read behavior should be the
         *             same as read( b[], off, len ). Timeouts have not been well tested.
         */

        public synchronized int read(final byte b[], final int off, final int len, final byte t[]) throws IOException {
            if (debug_read) z.reportln("RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") called" /* + new String(b) */ );
            int result;
            /*
             * Some sanity checks
    ...
    class SerialOutputStream extends OutputStream {
        ...
        */
        ...
        public void write(final byte b[]) throws IOException {
            if (debug_write) z.reportln("Entering RXTXPort:SerialOutputStream:write(" + b.length + ") "/* + new String(b) */ );
            if (speed == 0) return;
            if (monThreadisInterrupted == true) return;
            if (fd == 0) throw new IOException();
            synchronized (IOLockedMutex) {
                IOLocked++;
            }
        ...
        public void write(final byte b[], final int off, final int len) throws IOException {
            if (speed == 0) return;
            if (off + len > b.length) throw new IndexOutOfBoundsException("Invalid offset/length passed to read");

            final byte send[] = new byte[len];
    ...
    protected final static boolean debug = Boolean.getBoolean("debug");
    protected final static boolean debug_read = Boolean.getBoolean("debug_read");
    protected final static boolean debug_read_results = Boolean.getBoolean("debug_read_results");
    protected final static boolean debug_write = Boolean.getBoolean("debug_write");
    protected final static boolean debug_events = Boolean.getBoolean("debug_events");
    protected final static boolean debug_verbose = Boolean.getBoolean("verbose");
     ...
     *
     */
    public static int staticGetBaudRate(final String port) throws UnsupportedCommOperationException {
     ...
     *
     */
    public static int staticGetDataBits(final String port) throws UnsupportedCommOperationException {
     ...
     *
     */
    public static int staticGetParity(final String port) throws UnsupportedCommOperationException {
     ...
     *
     */
    public static int staticGetStopBits(final String port) throws UnsupportedCommOperationException {
    ...
    public static boolean staticIsCD(final String port) throws UnsupportedCommOperationException {
    ...
    public static boolean staticIsCTS(final String port) throws UnsupportedCommOperationException {
    ...
    public static boolean staticIsDSR(final String port) throws UnsupportedCommOperationException {
    ...
    public static boolean staticIsDTR(final String port) throws UnsupportedCommOperationException {
    ...
    public static boolean staticIsRI(final String port) throws UnsupportedCommOperationException {
    ...
    public static boolean staticIsRTS(final String port) throws UnsupportedCommOperationException {
     ...
     * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
     *
     * Open the port and set DSR. remove lockfile and do not close This is so some software can appear to set the DSR before 'opening' the port a second time
     * later on.
     *
    ...
    public static boolean staticSetDSR(final String port, final boolean flag) throws UnsupportedCommOperationException {
     ...
     * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
     *
     * Open the port and set DTR. remove lockfile and do not close This is so some software can appear to set the DTR before 'opening' the port a second time
     * later on.
     *
     * @return true on success
     * @throws UnsupportedCommOperationException;
    ...
    public static boolean staticSetDTR(final String port, final boolean flag) throws UnsupportedCommOperationException {
     ...
     * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
     *
     * Open the port and set RTS. remove lockfile and do not close This is so some software can appear to set the RTS before 'opening' the port a second time
     * later on.
     *
     * @return none
     * @throws UnsupportedCommOperationException;
    ...
    public static boolean staticSetRTS(final String port, final boolean flag) throws UnsupportedCommOperationException {
    ...
    public static void staticSetSerialPortParams(final String f, final int b, final int d, final int s, final int p) throws UnsupportedCommOperationException {
    ...
    */
    ...
    @Override
    public void addEventListener(final SerialPortEventListener lsnr) throws TooManyListenersException {
        /*
    ...
    public void close() {
        synchronized (this) {
            if (debug) z.reportln("RXTXPort:close( " + name + " )");
        ...
        setDTR(false);
        setDSR(false);
        if (debug) z.reportln("RXTXPort:close( " + name + " ) setting monThreadisInterrupted");
        if (!monThreadisInterrupted) removeEventListener();
        if (debug) z.reportln("RXTXPort:close( " + name + " ) calling nativeClose");
        nativeClose(name);
        if (debug) z.reportln("RXTXPort:close( " + name + " ) calling super.close");
        super.close();
        fd = 0;
        closeLock = false;
        if (debug) z.reportln("RXTXPort:close( " + name + " ) leaving");
    }
    ...
    */
    ...
    public boolean isReceiveThresholdEnabled() {
        if (debug) z.reportln("RXTXPort:isReceiveThresholdEnable() called and returning" + (threshold > 0));
        return threshold > 0;
    }
    ...
    public boolean isReceiveTimeoutEnabled() {
        if (debug) z.reportln("RXTXPort:isReceiveTimeoutEnabled() called and returning " + NativeisReceiveTimeoutEnabled());
    ...
    public void removeEventListener() {
        ...
        if (monThreadisInterrupted == true) {
            ...
            if (debug && monThread.isAlive()) z.reportln("	MonThread is still alive!");
     ...
     */
    public boolean sendEvent(final int event, final boolean state) {
        if (debug_events) z.report("RXTXPort:sendEvent(");
        /* Let the native side know its time to die */

        if (fd == 0 || SPEventListener == null || monThread == null) return true;
        ...
        final SerialPortEvent e = new SerialPortEvent(this, event, !state, state);
    ...
    @Override
    public boolean setBaudBase(final int BaudBase) throws UnsupportedCommOperationException, IOException {
    ...
    @Override
    public boolean setCallOutHangup(final boolean NoHup) throws UnsupportedCommOperationException {
    ...
    @Override
    public boolean setDivisor(final int Divisor) throws UnsupportedCommOperationException, IOException {
    ...
    @Override
    public synchronized void setSerialPortParams(final int b, final int d, final int s, final int p) throws UnsupportedCommOperationException {
     ...
     */
    @Override
    public boolean setUARTType(final String type, final boolean test) throws UnsupportedCommOperationException {
     ...
     *
     * This is important for applications that flicker the Monitor thread while keeping the port open. In worst case test cases this loops once or twice every
     * time.
     */
