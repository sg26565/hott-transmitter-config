    ...
    public static Font getFont(final String name, final int size, final int style, final boolean strikeout, final boolean underline) {
        ...
        if (strikeout || underline) try {
            ...
            if (lf != null && lfCls != null) {
                if (strikeout) lfCls.getField("lfStrikeOut").set(lf, new Byte((byte) 1)); //$NON-NLS-1$
                if (underline) lfCls.getField("lfUnderline").set(lf, new Byte((byte) 1)); //$NON-NLS-1$
            }
